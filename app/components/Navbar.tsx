            ...
            {navLinks.map((link) => (
              <div key={link.name} className="relative group">
                <Link
                  href={link.href}
                  className={cn(
                    'relative font-body text-sm font-medium transition-colors duration-200 flex items-center gap-1',
                    link.highlight
                ...
                >
                  ...
                  <span
                    className={cn(
                      'absolute left-0 -bottom-1 w-full h-[2px] scale-x-0 origin-left transition-transform duration-300 group-hover:scale-x-100',
                      link.highlight ? 'bg-yellow-400' : 'bg-kestone-red',
                    )}
                  />
                </Link>
              </div>
              ...
              user ? (
                <div className="relative" ref={dropdownRef}>
                  ...
                  >
                    ...
                    <ChevronDown
                      size={14}
                      className={cn('text-white/50 transition-transform duration-200', dropdownOpen && 'rotate-180')}
                    />
                  </button>
                        ...
                        <div className="py-1.5">
                          {isAdmin && (
                            <Link
                              ...
                              className="flex items-center gap-3 px-4 py-2.5 text-blue-400 hover:text-blue-300 hover:bg-blue-500/5 text-sm transition-colors"
                            >
                              <LayoutDashboard size={15} /> Admin Dashboard
                            </Link>
                          )}
                          <button
                            ...
                            className="w-full flex items-center gap-3 px-4 py-2.5 text-red-400 hover:text-red-300 hover:bg-red-500/5 text-sm transition-colors"
                          >
                            <LogOut size={15} /> Sign Out
                          </button>
                        </div>
          ...
          <div className="lg:hidden flex items-center gap-3">
            {!loading && !user && (
              <button
                ...
                aria-label="Sign in with Google"
              >
                {googleLoading ? <Loader2 size={22} className="animate-spin" /> : <LogIn size={22} />}
              </button>
            )}
            <button
              ...
              <Link
                ...
                className={cn(
                  ...
                  link.highlight
                    ...
                    : 'text-white hover:text-kestone-red',
                )}
              >
                {link.name}
              </Link>
                  ...
                  <>
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/10">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-white/40 text-xs truncate">{user.email}</p>
                        <span className={cn(
                          'inline-flex items-center gap-1 mt-1 text-[10px] font-bold uppercase tracking-wider rounded-full px-2 py-0.5',
                          isAdmin
                            ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30'
                            : 'bg-white/10 text-white/50 border border-white/15',
                        )}>
